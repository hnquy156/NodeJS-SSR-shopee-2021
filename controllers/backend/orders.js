...
const pageTitle = "Orders Management";

// GET VIEW ORDER
router.get('/view/:id', async (req, res) => {
	const id = req.params.id;
	res.locals.sidebarActive = `${collectionName}|view`;

	const orderItem = await MainModel.getItem(id);
	
	res.render(`${folderView}/view`, { 
		pageTitle,
...
router.get('/change-status/:status/:id', async (req, res) => {
	const currentStatus = ParamsHelpers.getParam(req.params, 'status', 0);
	const id		    = ParamsHelpers.getParam(req.params, 'id', '');
	const user 			= req.user;
...
router.get('(/status/:status)?', async (req, res, next) => {
	...
	const filterStatus = await UtilsHelpers.createFilterOrderStatus(currentStatus, collectionName, condition);
