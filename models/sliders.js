...
const ItemsModels = require(__path_schemas + 'sliders');
...
module.exports = {
    ...
    getListFrontend: async (options = null, params = null) => {
        ...
        let select = 'name thumb ordering';
        let sort = 'ordering';
        let skip = null;
        let limit = null;

        if (options.task === 'list') {
            return ItemsModels.find(condition).select(select).sort(sort).skip(skip).limit(limit);
        }
